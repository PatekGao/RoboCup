    ...
    void MainWindow::on_button_connect_clicked(bool check) {
        if (ui.identifyRoundOne->isChecked() || ui.identifyRoundTwo->isChecked() || ui.measureRoundOne->isChecked() ||
            ...
            if (ui.identifyRoundOne->isChecked()) {
                qnode.log(QNode::Info, "Identify Round 1 start!");
                qnode.startIdentify(true, "identify");
            } else if (ui.identifyRoundTwo->isChecked()) {
                qnode.log(QNode::Info, "Identify Round 2 start!");
                qnode.startIdentify(false, "identify");
            } else if (ui.measureRoundOne->isChecked()) {
                qnode.log(QNode::Info, "Measure Round 1 start!");
                qnode.startIdentify(true, "measure1");
            } else if (ui.measureRoundTwo->isChecked()) {
                qnode.log(QNode::Info, "Measure Round 2 start!");
                qnode.startIdentify(false, "measure2");
            }
        } else {
...
// 深度图像显示
    void MainWindow::depthImageDisplay(cv::Mat &raw) {
        QImage img;
        img = QImage((const unsigned char *) (raw.data),
                     raw.cols, raw.rows,
                     raw.cols * raw.channels(),
    ...
    void MainWindow::updateStatusShow(int step, bool *sss) {
        switch (step) {
            case -1:
                ...
                break;
            case 0:
                ui.status_runing->setText("Waiting");
                break;
            case 1:
                ui.status_runing->setText("Identifying desk");
                break;
            case 2:
                ui.status_runing->setText("Waiting for turning");
                break;
            case 3:
